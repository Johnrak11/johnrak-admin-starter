    ...
    <Card>
      <template #header>
        <div class="min-w-0 break-words">
          <div class="font-semibold truncate">LinkedIn Import (JSON)</div>
      ...
      <div class="space-y-3 text-sm text-muted-foreground min-w-0">
        <div class="rounded-xl border border-border bg-muted/30 p-4 min-w-0">
          <div class="font-medium text-foreground">Example JSON</div>
          <pre
            class="mt-2 overflow-auto rounded-lg border border-border bg-muted/30 p-3 text-xs text-foreground max-h-[300px]"
            >{{ example }}</pre
          >
        </div>
        ...
        <textarea
          v-model="jsonText"
          class="min-h-[220px] w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
          placeholder="{ ... }"
        />

        <div class="flex items-center gap-2">
          <Button @click="importNow" :disabled="loading">{{
        ...
        <div
          v-if="result"
          class="rounded-xl border border-border bg-muted/30 p-4 max-w-full overflow-hidden"
        >
          <div class="font-medium text-foreground">Result</div>
          <pre
            class="mt-2 overflow-auto text-xs text-foreground max-h-[300px]"
            >{{ result }}</pre
          >
        </div>
      </div>
